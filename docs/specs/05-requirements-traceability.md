...
| `CTX-001..012` 客户/商机/跟进/任务上下文读取、匹配与来源 | P1-CTX | Partial | 机器人和 Web 工作流均覆盖本人 Base/Task 查询过滤、客户/商机多匹配、失败降级、只读副作用、上下文注入、版本持久化回读、来源展示，以及商机/跟进行动字段冲突保留与较新来源标记；历史数据评测待补 | 真实飞书/Web 查询、来源和多分支 UI 待验收 |
