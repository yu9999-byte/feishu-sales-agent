...
最新可信沟通时间和无效记录跳过规则；与判定层合计 Agent `215/215` 通过。该能力仍是纯
内存、无外部副作用，不能替代真实分页读取、Task 权限、扫描、去重或提醒投递。
...
| `S4-001` 商机 7 天未更新提醒 | P1 internal trigger | Partial / disabled / prerequisites open | 纯判定层 6 项测试覆盖可信时间、7 天边界、负责人、任务来源和免打扰；新增汇总服务 4 项测试覆盖按商机取最新及无效记录跳过；尚无真实扫描、持久去重、定时任务或提醒发送 | 先开通 Task 读权限并接入真实跟进分页读取，再接入默认禁用的扫描/去重/本人投递与真实验收 |
