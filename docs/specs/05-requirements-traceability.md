...
状态：`Approved / Goal v2 aligned / automated Green / UI regression pending 2026-09-25`
...
| `CTX-001..012` 客户/商机/跟进/任务上下文读取、匹配与来源 | P1-CTX | Partial | 3 个定向测试文件覆盖本人 Base/Task 查询过滤、客户/商机多匹配、失败降级、只读副作用、跟进上下文注入、来源持久化、确认卡展示，以及商机/跟进行动字段冲突保留与较新来源标记；历史数据评测和 Postgres 持久化回读待补 | 真实飞书查询/来源 UI 待验收；Web 独立草案版本/API 契约尚未接入 |
