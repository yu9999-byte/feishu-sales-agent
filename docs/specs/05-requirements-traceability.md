...
2026-09-27 新增独立既有商机状态更新网关：目标必须是唯一记录 ID，执行前校验当前销售和
确认时旧状态，只提交生命周期字段并写后回读。真实只读清单确认 5 条本人商机状态均为空；
本轮没有批量回填，聊天澄清、确认卡、逐条审计和历史可信时间治理仍待接入。
...
| `S4-001` 商机 7 天未更新提醒 | P1 internal trigger | Partial / scan green / disabled / prerequisites open | 纯判定、逐商机汇总、本人商机/跟进/Task 全分页与扫描编排均有测试；默认关闭且只返回候选/跳过原因，来源不完整整轮 fail closed；Task 权限及独立生命周期字段/四类值映射均已真实回归。新建商机按租户配置初始化进行中，已有商机更新不触碰状态；5 条历史商机保持空状态并安全跳过。新增逐条状态更新网关，但尚无历史可信时间、持久去重、定时任务或提醒发送 | 接入状态确认卡，逐条分类既有商机并处理历史可信时间；再做持久去重、默认关闭触发器、本人投递和真实验收 |
