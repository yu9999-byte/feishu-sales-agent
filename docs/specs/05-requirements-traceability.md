...
商机状态均为空；本轮没有批量回填，真实飞书 UI 和写后对账仍待复验。
...
| `S4-001` 商机 7 天未更新提醒 | P1 internal trigger | Partial / scan green / disabled / prerequisites open | 纯判定、逐商机汇总、本人商机/跟进/Task 全分页与扫描编排均有测试；默认关闭且只返回候选/跳过原因，来源不完整整轮 fail closed；Task 权限及独立生命周期字段/四类值映射均已真实回归。新建商机按租户配置初始化进行中，已有商机更新不触碰状态；5 条历史商机保持空状态并安全跳过。状态聊天确认闭环已自动化接入，但尚无历史可信时间、持久去重、定时任务或提醒发送 | 逐条完成历史状态确认并处理历史可信时间；再做持久去重、默认关闭触发器、本人投递和真实验收 |
