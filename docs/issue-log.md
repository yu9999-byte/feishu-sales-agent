...
| `ISS-S4-001` | “商机超过 7 天未更新”的主动提醒尚未形成闭环 | Partial / disabled / prerequisites open | 纯判定层和 6 项测试已实现；真实跟进表已新增并验证“本次沟通发生时间”（`fld28D7L08`），未来确认跟进仅写入明确时间，历史记录不自动回填；商机级有效时间汇总、Task 读取权限、扫描、去重、真实提醒和触发器仍未完成 | 开通应用身份 Task 读权限；定义商机级有效跟进汇总后，再接入默认关闭的扫描、持久去重与本人提醒 |
